...
* Web 前端Library基于 Vue 2.x，UI Library主要基于 Vux 2.x，构建依赖基于 npm
...
Web 前端从整体技术栈选择上，可以看出这是一个稳健并且有一定前瞻性的技术方案。基于成熟的 Vue.js 生态体系，UI 设计层面基于 WeUI ，所以前端 UI 框架基于 Vux Library。Vue.js 2.x 版本在性能上提升了很多，完全面向现代浏览器，可以直接使用 ECMAScript 6 来开发。内部已经写好并整合的方案主要包括：
...
整套架构 Server 端与 Web 前端是完全分离，在 Server 端编写 REST API，Web 项目则是完全的 Web App，而不是通过模板来耦合。web 中是 Web 项目的全部代码，是完全独立的一套体系，也可以提出去，单独项目维护。
...
* 系统依赖 nodejs 环境为 4.6.x 版本，如果启动遇到错误，请首先确保 nodejs 版本
...
$ npm run dev
```
运行 web 端环境，通过 `http://localhost:8080` 可以调试
...
* web 目录完全可以独立，是一套完整的 Web 前端开发结构，本身也支持跨域方案，所以也可以 Web 与 Server 分工开发
